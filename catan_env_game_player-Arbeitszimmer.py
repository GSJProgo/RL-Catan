...
def pretty_board_update():
    print("pretty board roads and settlements: first number (1 = settlements,3 = roads), second number(1 = player0, 2 = player1)")
    ...
    for i in range (0,11,1):
        for j in range(0,21,1):
            ...
            if (board.settlements_available[i][j] == 1):
                board.prettyboard[i][j] = 1
            
            if (board.roads_available[i][j] == 1):
                board.prettyboard[i][j] = 3
...
def settlement_place_placement(a,b):
    player = players[game.cur_player]
    settlement_possible = ((1-board.ZEROBOARD)*(1-player0.roads)*(1-player1.roads))
    if settlement_possible[a,b] == 1:
        player.settlements[a][b] = 1
        tile_update_rewards(a,b)
        player.victorypoints += 1
        return 1 
...
def trav(i,j,roads_board):
    #it might be an idea to use this for other function too, but let's see
    n = 11 
    m = 21
    player = players[game.cur_player]
    if i < 0 or j < 0 or i >= n or j >= m or roads_board[i][j] == 0: return 0
    roads_board[i][j] = 0
    return 1 + trav(i+2, j,roads_board) + trav(i+1, j+1,roads_board) + trav(i+1, j-1,roads_board) + trav(i, j+2,roads_board) + trav(i-1, j+1,roads_board) + trav(i-2, j,roads_board) + trav(i-1, j-1,roads_board) + trav(i, j-2,roads_board) 
def find_longest_road():
    player = players[game.cur_player]
    roads_board = player.roads * (1-board.ZEROBOARD)
    ans, n, m = 0, len(roads_board), len(roads_board[0])
    for i, j in product(range(n), range(m)):
        if roads_board[i][j]: ans = max(ans, trav(i, j,roads_board))
...
def player_place_placement_phase():
    ...
    while possible == 0:
        a = 0
        b = 0
        while a not in range(1,11) or b not in range(1,21):
    ...
    while possible == 0:
        c = 0
        d = 0
        while c not in range(1,11) or d not in range(1,21):
...
def turn_starts():
    ...
    while a != "et":
        ...
        if a == "ay":
            ...
            possible = activate_yearofplenty_func(e,f)
            if possible == 0:
