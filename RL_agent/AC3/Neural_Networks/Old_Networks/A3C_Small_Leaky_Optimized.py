...
class ActorCritic(nn.Module):
    ...
    def loss_func(self, boardstate, vectorstate, a, v_t, device, total_step):
        ...
        total_loss = (c_loss * 10**3 + a_loss * 40 + entropy * 2 * 10 ** -3 + l2_activity_loss * 5 * 10 ** -4).mean()
        return values, total_loss, c_loss.mean() * 10 ** 3, a_loss.mean() * 40, entropy.mean() * 2 * 10 ** -3, l2_activity_loss.mean() * 2 * 10 ** -4
